...
<?php $a=sw_get_current_weekday(); echo $a; ?>
...
<table class="table text-center badge-light table-hover">
    ...
    <tbody>
        <?php
        $sql123="select * from hoadondiennuoc where MaPhong ='$map' order by Thang DESC";
        ...
        <tr>
            ...
            <td>
                <?php if($row123['TinhTrang']==='đã thu') {
            ...
            <td>
                <a href="index.php?action=quanlydiennuoc&view=them&map=<?php echo $map; ?>&delete=<?php echo $row123['MaHD']; ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa hóa đơn này?');">Xóa</a>
            </td>
			<td>
                <a href="index.php?action=quanlydiennuoc&view=sua&map=<?php echo $map; ?>&mahd=<?php echo $row123['MaHD']; ?>"onclick="return confirm('Bạn có chắc chắn muốn sửa hóa đơn này?');">Sửa</a>
			</td>
        </tr>
        <?php } ?>
    </tbody>
...
if(isset($_GET['delete'])){
    $mahd=$_GET['delete'];
    $sql="DELETE FROM hoadondiennuoc WHERE MaHD='$mahd'";
    $rs=mysqli_query($conn,$sql);
    if($rs){
        $link="index.php?action=quanlydiennuoc&view=them&tb=1&map=".$map;
        header('Location:'.$link);
...
if (isset($_GET['action']) && $_GET['action'] == 'quanlydiennuoc') {
    if (isset($_GET['view']) && $_GET['view'] == 'sua') {
        include 'sua.php';
    }
}
