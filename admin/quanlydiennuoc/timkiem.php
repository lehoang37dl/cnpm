...
if(isset($_POST['btsearch'])){
	...
	$rs=mysqli_query($conn,$sql); ?>
		<table class="table-hover table text-center">
			<thead>
				<tr class="badge-info">
					<th>#</th><th>Phòng</th><th>Khu</th><th  class="badge-danger" ></th>
				</tr>
			</thead>
<?php $i=0; while ($row=mysqli_fetch_array($rs)) {$i=$i+1; ?>
		<tbody>
				<tr>
					<td><?php echo $i ;?></td><td><?php echo $row['MaPhong'] ;?></td><td><?php echo $row['MaKhu'] ;?></td><td><a href="index.php?action=quanlydiennuoc&view=them&map=<?php echo $row['MaPhong'] ?>" ><i class="fas fa-location-arrow"></i> Thêm</a></td>
				</tr>
			</tbody>
